...
**На Revit 2027 картина другая и измерена пробой.** Собственные `Microsoft.Extensions.*` там
лежат не в корне, а в `AddIns\IssuesManagement` и `AddIns\ExtendedAPIs`, и версия у них
**8.0.0.0**. Часть из них выигрывает загрузку у 10.0.11 из shared framework: в живом процессе
одновременно присутствовали и `Configuration` 8.0.0.0 из ревитовской папки, и
`Configuration.Json` 10.0.0.0 из `Microsoft.NETCore.App`. То есть правило «не ниже 2.2» описывает
только нижний край диапазона; на верхнем краю опасная граница — **8.0.0.0**.
...
`Microsoft.Extensions.AI.Abstractions` 10.0.0.0 из `AddIns\Assistant`. Прежде чем строить свой
модуль MCP, стоит выяснить, что этот сервер уже умеет и расширяем ли он — возможно, задача
сводится к регистрации инструментов в чужом сервере, а не к своему.
...
#   Оговорка по 2024: запись доверия снимает окно на 2025-2027, но НЕ на 2024 — там оно
#   появляется каждый раз и требует ответа руками. Для такого запуска есть --allow-untrusted.
#   Настоящее лекарство — подписать сборку Authenticode.
