...
> **Механика .NET Framework из `..\BHS` и `..\BimHouseApp` перенесена — она обязательна.**
> Подмена `TargetFrameworkProfile` и `FrameworkPathOverride` через `ToolLocationHelper` нужна
> потому, что у .NETFramework нет понятия целевой платформы: Revit-часть моникёра приходится
> нести профилем, а профиль подразумевает каталог reference-сборок, которого не существует.
>
> Ранее здесь стояло обратное утверждение — что перенос был бы работой впустую. Под прежним
> дизайном, где моникёр транслировался в обычный `net48`, оно было верным. Дизайн сменился:
> трансляция делала проект невидимым для IDE, и от неё отказались (см. `readme.md` SDK,
