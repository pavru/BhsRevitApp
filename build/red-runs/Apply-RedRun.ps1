...
.PARAMETER Run
    Which run to apply, 1..9. Runs 5-8 are the second round; run 9 is the mode without additional boxes. See readme.md for what each one proves.
...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true)][ValidateRange(1, 9)][int] $Run,
    [switch] $Revert,
    [switch] $List
)
