...
.PARAMETER Run
    Which run to apply, 1..10. Runs 5-8 are the second round; run 9 is the mode without additional boxes; run 10 is the length breakdown; runs 11-12 are the check for stale lengths; runs 13-15 are the third round, behind what a small breakage could not reach; run 16 is InLinks counting elements. See readme.md for what each one proves.
...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true)][ValidateRange(1, 16)][int] $Run,
    [switch] $Revert,
    [switch] $List
)
