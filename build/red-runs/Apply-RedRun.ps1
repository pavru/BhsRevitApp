...
.PARAMETER Run
    Which run to apply, 1..22. Runs 5-8 are the second round; run 9 is the mode without additional boxes; run 10 is the length breakdown; runs 11-12 are the check for stale lengths; runs 13-15 are the third round, behind what a small breakage could not reach; run 16 is InLinks counting elements; runs 17-18 are the carrier catalogue and its filter; runs 19-22 are the installation methods. See readme.md for what each one proves.
...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true)][ValidateRange(1, 22)][int] $Run,
    [switch] $Revert,
    [switch] $List
)
